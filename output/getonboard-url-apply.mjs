...
async function labelOf(locator) {
  return locator.evaluate((node) => {
    ...
    const block = node.closest('label, fieldset, .form-group, .field, div, section');
    return [node.getAttribute('aria-label'), node.getAttribute('placeholder'), label?.textContent, block?.textContent]
...
function expectedKind(question, fallback = 'text') {
  ...
  if (/selecciona una opcion|yes\s+no|si\s+no|s[ií]\s+no/.test(q)) return 'select';
  if (/cuantos anos|cuanto tiempo|anos de experiencia|tiempo de experiencia|nivel.*1.*6|1.*6|a1.*c2|c2/.test(q)) return 'number';
  if (/(sueldo|salario|renta|pretension|expectativa|remuneracion)/.test(q) && /(usd|dolares|dólares|\$|numero|indique|ingrese)/.test(q)) return 'number';
...
function yesNoFor(question) {
  const q = norm(question);
  if (/residencia legal.*chile|vive en chile|trabajar en chile/.test(q)) return 'Si';
  ...
  if (/react|typescript|javascript|python|sql|api|rest|git|github|linux|debian|arch|automatizacion|frontend|front-end|datos/.test(q)) return 'Si';
  if (/hibrid|presencial|santiago|las condes|providencia/.test(q)) return 'Si';
...
function numberFor(question) {
  const q = norm(question);
  if (/usd|dolares|dólares/.test(q) && /(sueldo|salario|renta|pretension|expectativa|remuneracion)/.test(q)) return '900';
  if (/(sueldo|salario|renta|pretension|expectativa|remuneracion)/.test(q)) return '900000';
  ...
  if (/python|typescript|javascript|react|sql|api|rest|git|github|linux|debian|arch|bash|automatizacion|frontend|front-end|datos|qa/.test(q)) return '1';
  return '1';
}
...
async function fillForm(page) {
  ...
  for (let i = 0, count = await textareas.count().catch(() => 0); i < count; i += 1) {
    ...
    await setValue(field, answerFor(label, text, 'text'));
  }
  ...
  for (let i = 0, count = await inputs.count().catch(() => 0); i < count; i += 1) {
    ...
    if (!label) continue;
    await setValue(input, answerFor(label, text, 'short'));
  }
  ...
  for (let i = 0, count = await selects.count().catch(() => 0); i < count; i += 1) {
    ...
    let pick = yesNo === 'No' ? no : yesNo === 'Si' ? yes : yes;
    if (/senior|avanzado|experiencia laboral formal|gcp|salesforce/.test(label)) pick = no || yes;
    pick ||= options.find((o) => o.value && !/selecciona|select|choose/i.test(o.text));
    if (pick?.value) await select.selectOption(pick.value, { timeout: 1000 }).catch(() => {});
  }
...
async function applyOne(context, url) {
  ...
  if (/ya postulaste|postulacion enviada|postulación enviada/i.test(text)) return { title, status: 'already-applied' };
  ...
  for (let step = 0; step < 8; step += 1) {
    ...
    if (/vista previa|review|enviar postulaci[oó]n ahora|submit application/i.test(text) || /step=(preview|review)/i.test(page.url())) {
      ...
      if (sent) return { title, status: 'submitted' };
      return { title, status: submitted ? 'submitted-unconfirmed' : 'preview-no-submit-button' };
    }
    const next = await clickText(page, [/^siguiente$/, /^continuar$/, /guardar y continuar/, /^vista previa$/]);
    if (!next) return { title, status: 'form-no-next' };
    await page.waitForTimeout(2500);
  }
